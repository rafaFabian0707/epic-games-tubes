...
@section('breadcrumb','Edit Diskon')
...
<div class="flex items-center gap-4 mb-6">
    ...
    <div>
        <h1 class="text-xl font-bold text-white">Edit Diskon</h1>
        <p class="text-gray-500 text-xs mt-0.5">
            {{ $discount->game->title ?? '—' }} ·
            <span class="text-orange-400 font-semibold">{{ number_format($discount->discount_pct, 0) }}% OFF</span>
        </p>
    </div>
</div>
...
<form method="POST" action="{{ route('admin.discounts.update', $discount->discount_id) }}">
    @csrf @method('PUT')
